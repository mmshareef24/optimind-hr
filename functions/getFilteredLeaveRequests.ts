...
/**
 * Get leave requests based on user role and access permissions
...
Deno.serve(async (req) => {
    try {
        ...
        // Get all employees and leave requests
        const allEmployees = await base44.asServiceRole.entities.Employee.list();
        const allLeaveRequests = await base44.asServiceRole.entities.LeaveRequest.list('-created_date');
        ...
        // ADMIN/HR USERS
        if (user.role === 'admin') {
            accessibleRequests = allLeaveRequests;
        ...
        // MANAGER USERS - See direct reports' requests
            ...
            accessibleRequests = allLeaveRequests.filter(req => 
        ...
        if (filters.leave_type) {
            accessibleRequests = accessibleRequests.filter(r => r.leave_type === filters.leave_type);
        }
        ...
        return Response.json({ 
            success: true,
            leave_requests: enrichedRequests,
