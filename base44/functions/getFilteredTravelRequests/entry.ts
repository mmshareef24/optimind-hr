...
/**
 * Get travel requests based on user role and access permissions
 * - Admin/HR/Finance: Can see all requests or filtered by company/department
...
Deno.serve(async (req) => {
    try {
        ...
        // Get all employees and travel requests
        const allEmployees = await base44.asServiceRole.entities.Employee.list();
        const allTravelRequests = await base44.asServiceRole.entities.TravelRequest.list('-created_date');
        ...
        if (user.role === 'admin') {
            accessibleRequests = allTravelRequests;
        ...
        // MANAGER USERS - See direct reports' requests
            ...
            accessibleRequests = allTravelRequests.filter(req => 
        ...
        if (filters.request_type) {
            accessibleRequests = accessibleRequests.filter(r => r.request_type === filters.request_type);
        }
        ...
        return Response.json({ 
            success: true,
            travel_requests: enrichedRequests,
