...
export default function PublicHolidays() {
  ...
  const initHolidaysMutation = useMutation({
    ...
    onSuccess: (data) => {
      ...
      toast.success(`${data.holidays_created} holidays initialized for ${data.year}`);
    },
    onError: (error) => {
      toast.error(error.response?.data?.message || 'Failed to initialize holidays');
    }
  });
  ...
  return (
    <div className="p-6 lg:p-8 space-y-6">
      ...
      <div className="flex flex-col lg:flex-row justify-between items-start lg:items-center gap-4">
        <div>
          <h1 className="text-3xl font-bold text-slate-900 mb-2">Public Holidays</h1>
          <p className="text-slate-600">Manage Saudi Arabian national and Islamic holidays</p>
        </div>
      ...
      {todayHoliday && (
        <Alert className="border-emerald-200 bg-gradient-to-r from-emerald-50 to-green-50">
          ...
          <AlertDescription className="text-emerald-900">
            <strong className="text-lg">🎉 Today is {todayHoliday.holiday_name}!</strong>
            <p className="text-sm mt-1">{todayHoliday.holiday_name_ar} • {todayHoliday.description}</p>
          </AlertDescription>
        </Alert>
      )}
      ...
      <Tabs defaultValue="all" className="space-y-6">
        <TabsList className="bg-white border-2 border-slate-200 p-1">
          ...
          <TabsTrigger value="islamic" className="data-[state=active]:bg-amber-600 data-[state=active]:text-white">
            Islamic ({islamicHolidays.length})
          </TabsTrigger>
        </TabsList>
      ...
      <Dialog open={showInitDialog} onOpenChange={setShowInitDialog}>
        <DialogContent>
          <DialogHeader>
            <DialogTitle>Initialize Holidays for {selectedYear}</DialogTitle>
          </DialogHeader>
          <Alert className="border-blue-200 bg-blue-50">
            <Calendar className="h-4 w-4 text-blue-600" />
            <AlertDescription className="text-blue-900 text-sm">
              This will automatically create official Saudi Arabian public holidays including:
              <ul className="list-disc ml-4 mt-2 space-y-1">
                <li>National Day (September 23)</li>
                <li>Foundation Day (February 22)</li>
                <li>Eid Al-Fitr (4 days)</li>
                <li>Arafat Day + Eid Al-Adha (5 days)</li>
              </ul>
              <p className="mt-2 text-xs">Islamic holiday dates are approximate and may vary by 1-2 days based on moon sighting.</p>
            </AlertDescription>
          </Alert>
          <DialogFooter>
            ...
            >
              {initHolidaysMutation.isPending ? 'Initializing...' : 'Initialize Holidays'}
            </Button>
          </DialogFooter>
        </DialogContent>
      </Dialog>
        ...
        <DialogContent className="max-w-2xl">
          ...
          <div className="space-y-4">
            <div className="grid md:grid-cols-2 gap-4">
              ...
              <div>
                <Label>Holiday Name (Arabic)</Label>
                <Input
      ...
      <CardContent className="p-6">
        <div className="flex justify-between items-start mb-4">
          ...
          <div className="flex gap-2">
            {isTodayHoliday && <Badge className="bg-emerald-500 text-white">Today!</Badge>}
            {isUpcoming && !isTodayHoliday && <Badge className="bg-blue-100 text-blue-700">Upcoming</Badge>}
            {isPassed && <Badge className="bg-slate-100 text-slate-600">Past</Badge>}
          </div>
        </div>

        <h3 className="font-bold text-slate-900 text-lg mb-1">{holiday.holiday_name}</h3>
        {holiday.holiday_name_ar && (
          <p className="text-sm text-slate-600 mb-3" dir="rtl">{holiday.holiday_name_ar}</p>
        )}
        ...
        {holiday.description && (
          <p className="text-xs text-slate-600 mb-3 p-2 bg-blue-50 rounded border-l-2 border-blue-400">
            {holiday.description}
          </p>
        )}

        <div className="flex gap-2 text-xs mb-3">
          <Badge variant="outline" className="capitalize">{holiday.holiday_type}</Badge>
          {holiday.is_paid && <Badge className="bg-emerald-100 text-emerald-700">Paid</Badge>}
          {holiday.is_islamic_calendar && <Badge className="bg-purple-100 text-purple-700">Hijri</Badge>}
        </div>
