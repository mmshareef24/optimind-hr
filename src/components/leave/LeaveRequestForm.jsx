...
import { Save, X, Upload, AlertCircle, Info, Calendar } from "lucide-react";
...
export default function LeaveRequestForm({ employee, leaveBalances = [], onSubmit, onCancel }) {
  ...
  useEffect(() => {
    if (formData.start_date && formData.end_date) {
      ...
      // Check balance validation after calculating days
      if (days > 0) {
        const balance = getBalance(formData.leave_type);
        if (balance < days && ['annual', 'sick'].includes(formData.leave_type)) {
